...
import Dashboard from './Dashboard';  // ← 추가
...
function ZoomOnClick() {
  useMapEvents({
    click(e) {
      e.target.setView(e.latlng, e.target.getZoom() + 1);
    }
  });
  return null;
}
...
function FlyToRegion({ target }) {
  ...
  useEffect(() => {
    if (target && GU_CENTERS[target]) {
      map.flyTo(GU_CENTERS[target], 13, { duration: 1 });
    }
  }, [target, map]);
  return null;
}
...
export default function App() {
  const [tab, setTab] = useState('map');  // ← 탭 상태 추가
  ...
  return (
    <div style={{ fontFamily:'sans-serif', background:'#E9E9E9', minHeight:'100vh', padding:'16px 20px' }}>
      ...
      <div style={{ display:'flex', justifyContent:'center', gap:8, marginBottom:16 }}>
        <button style={tabBtnStyle(tab === 'map')}       onClick={() => setTab('map')}>
          🗺️ 거주지 추천 지도
        </button>
